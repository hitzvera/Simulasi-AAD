...
class TaskRepository(private val tasksDao: TaskDao) {
    ...
    fun getTasks(filter: TasksFilterType): LiveData<PagedList<Task>> {
        Log.e("CHECK", filter.toString())
        val factory: DataSource.Factory<Int, Task> = tasksDao.getTasks(FilterUtils.getFilteredQuery(filter))
        return LivePagedListBuilder(factory, PagedList.Config.Builder().setPageSize(PAGE_SIZE).setEnablePlaceholders(
            PLACEHOLDERS).build()).build()
    }
